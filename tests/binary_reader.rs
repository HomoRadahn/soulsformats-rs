...
fn read_boolean() {
    for endian in [Endian::Little, Endian::Big] {
        ...
        assert_eq!(
            vec_reader.read_vec_bool(3).unwrap(),
            vec![true, false, true]
        );
        ...
        assert_eq!(get_reader.get_vec_bool(1, 2).unwrap(), vec![false, true]);
        assert_eq!(0, get_reader.position().unwrap());
    }
}
...
fn read_uint8() {
    for endian in [Endian::Little, Endian::Big] {
        ...
        let mut vec_reader = BinaryReader::from_bytes(bytes.clone(), endian, true);
        assert_eq!(vec_reader.read_vec_u8(3).unwrap(), vec![0x12, 0x13, 0x14]);

        let mut get_reader = BinaryReader::from_bytes(bytes.clone(), endian, true);
        assert_eq!(get_reader.get_u8(2).unwrap(), 0x14);
        assert_eq!(0, get_reader.position().unwrap());
        assert_eq!(get_reader.get_vec_u8(1, 2).unwrap(), vec![0x13, 0x14]);
        assert_eq!(0, get_reader.position().unwrap());
    }
}
...
fn read_uint16() {
    ...
    assert_eq!(
        little_vec_reader.read_vec_u16(2).unwrap(),
        vec![0x1312, 0x1514]
    );
    ...
    assert_eq!(
        little_get_reader.get_vec_u16(1, 2).unwrap(),
        vec![0x1413, 0x1615]
    );
    assert_eq!(0, little_get_reader.position().unwrap());
    ...
    assert_eq!(
        big_vec_reader.read_vec_u16(2).unwrap(),
        vec![0x1213, 0x1415]
    );
    ...
    assert_eq!(
        big_get_reader.get_vec_u16(1, 2).unwrap(),
        vec![0x1314, 0x1516]
    );
    assert_eq!(0, big_get_reader.position().unwrap());
}
...
fn read_uint32() {
    ...
    let mut little_vec_reader = BinaryReader::from_bytes(many.clone(), Endian::Little, true);
    assert_eq!(
        little_vec_reader.read_vec_u32(2).unwrap(),
    ...
    assert_eq!(0, little_get_reader.position().unwrap());
    assert_eq!(
        little_get_reader.get_vec_u32(1, 2).unwrap(),
    ...
    let mut big_vec_reader = BinaryReader::from_bytes(many.clone(), Endian::Big, true);
    assert_eq!(
        big_vec_reader.read_vec_u32(2).unwrap(),
    ...
    assert_eq!(0, big_get_reader.position().unwrap());
    assert_eq!(
        big_get_reader.get_vec_u32(1, 2).unwrap(),
...
fn read_uint64() {
    ...
    let mut little_vec_reader = BinaryReader::from_bytes(many.clone(), Endian::Little, true);
    assert_eq!(
        little_vec_reader.read_vec_u64(2).unwrap(),
    ...
    assert_eq!(0, little_get_reader.position().unwrap());
    assert_eq!(
        little_get_reader.get_vec_u64(0, 2).unwrap(),
        vec![0x1918171615141312, 0x2726252423222120]
    );
    assert_eq!(0, little_get_reader.position().unwrap());
    ...
    let mut big_vec_reader = BinaryReader::from_bytes(many.clone(), Endian::Big, true);
    assert_eq!(
        big_vec_reader.read_vec_u64(2).unwrap(),
    ...
    assert_eq!(0, big_get_reader.position().unwrap());
    assert_eq!(
        big_get_reader.get_vec_u64(0, 2).unwrap(),
        vec![0x1213141516171819, 0x2021222324252627]
    );
    assert_eq!(0, big_get_reader.position().unwrap());
}
...
fn read_int8() {
    ...
    for endian in [Endian::Little, Endian::Big] {
        ...
        let mut vec_reader = BinaryReader::from_bytes(bytes.clone(), endian, true);
        assert_eq!(vec_reader.read_vec_i8(3).unwrap(), vec![-2, 2, 127]);

        let mut get_reader = BinaryReader::from_bytes(bytes.clone(), endian, true);
        assert_eq!(get_reader.get_i8(1).unwrap(), 2);
        assert_eq!(0, get_reader.position().unwrap());
        assert_eq!(get_reader.get_vec_i8(0, 3).unwrap(), vec![-2, 2, 127]);
        assert_eq!(0, get_reader.position().unwrap());
    }
}
...
fn read_int16() {
    ...
    assert_eq!(
        little_vec_reader.read_vec_i16(3).unwrap(),
        vec![-2, 512, 32767]
    );
    ...
    assert_eq!(
        little_get_reader.get_vec_i16(0, 3).unwrap(),
        vec![-2, 512, 32767]
    );
    assert_eq!(0, little_get_reader.position().unwrap());
...
fn read_int32() {
    ...
    let mut little_vec_reader = BinaryReader::from_bytes(bytes.clone(), Endian::Little, true);
    assert_eq!(little_vec_reader.read_vec_i32(2).unwrap(), vec![-2, 131072]);
    let mut little_get_reader = BinaryReader::from_bytes(bytes.clone(), Endian::Little, true);
    assert_eq!(little_get_reader.get_i32(4).unwrap(), 131072);
    assert_eq!(
        little_get_reader.get_vec_i32(0, 2).unwrap(),
        vec![-2, 131072]
    );
    assert_eq!(0, little_get_reader.position().unwrap());
    ...
    let mut big_vec_reader = BinaryReader::from_bytes(bytes.clone(), Endian::Big, true);
    assert_eq!(
        big_vec_reader.read_vec_i32(2).unwrap(),
        vec![-16777217, 512]
    );
    let mut big_get_reader = BinaryReader::from_bytes(bytes, Endian::Big, true);
    assert_eq!(big_get_reader.get_i32(4).unwrap(), 512);
    assert_eq!(
        big_get_reader.get_vec_i32(0, 2).unwrap(),
        vec![-16777217, 512]
    );
    assert_eq!(0, big_get_reader.position().unwrap());
}
...
fn read_int64() {
    ...
    let mut big_vec_reader = BinaryReader::from_bytes(bytes.clone(), Endian::Big, true);
    assert_eq!(
        big_vec_reader.read_vec_i64(2).unwrap(),
        vec![-72057594037927937, 512]
    );
    let mut big_get_reader = BinaryReader::from_bytes(bytes, Endian::Big, true);
    assert_eq!(big_get_reader.get_i64(8).unwrap(), 512);
    assert_eq!(
        big_get_reader.get_vec_i64(0, 2).unwrap(),
        vec![-72057594037927937, 512]
    );
    assert_eq!(0, big_get_reader.position().unwrap());
}
...
fn read_float32() {
    ...
    assert_eq!(little_vec_reader.read_vec_f32(2).unwrap(), vec![1.0, -2.5]);
    ...
    assert_eq!(
        little_get_reader.get_vec_f32(0, 2).unwrap(),
        vec![1.0, -2.5]
    );
    assert_eq!(0, little_get_reader.position().unwrap());
    ...
    assert_eq!(big_vec_reader.read_vec_f32(2).unwrap(), vec![1.0, -2.5]);
    let mut big_get_reader = BinaryReader::from_bytes(big_bytes, Endian::Big, true);
    assert_eq!(big_get_reader.get_f32(4).unwrap(), -2.5);
    assert_eq!(big_get_reader.get_vec_f32(0, 2).unwrap(), vec![1.0, -2.5]);
    assert_eq!(0, big_get_reader.position().unwrap());
}
...
fn read_float64() {
    ...
    let mut little_vec_reader =
        BinaryReader::from_bytes(little_bytes.clone(), Endian::Little, true);
    assert_eq!(little_vec_reader.read_vec_f64(2).unwrap(), vec![1.0, -2.5]);
    let mut little_get_reader = BinaryReader::from_bytes(little_bytes, Endian::Little, true);
    assert_eq!(little_get_reader.get_f64(8).unwrap(), -2.5);
    assert_eq!(
        little_get_reader.get_vec_f64(0, 2).unwrap(),
        vec![1.0, -2.5]
    );
    assert_eq!(0, little_get_reader.position().unwrap());
    ...
    let mut big_vec_reader = BinaryReader::from_bytes(big_bytes.clone(), Endian::Big, true);
    assert_eq!(big_vec_reader.read_vec_f64(2).unwrap(), vec![1.0, -2.5]);
    let mut big_get_reader = BinaryReader::from_bytes(big_bytes, Endian::Big, true);
    assert_eq!(big_get_reader.get_f64(8).unwrap(), -2.5);
    assert_eq!(big_get_reader.get_vec_f64(0, 2).unwrap(), vec![1.0, -2.5]);
    assert_eq!(0, big_get_reader.position().unwrap());
}
...
fn read_varint_vectors() {
    ...
    assert_eq!(short_reader.read_vec_varint(2).unwrap(), vec![-2, 2]);
    let mut short_get_reader = BinaryReader::from_bytes(short_bytes, Endian::Little, false);
    assert_eq!(short_get_reader.get_vec_varint(0, 2).unwrap(), vec![-2, 2]);
    ...
    assert_eq!(long_reader.read_vec_varint(2).unwrap(), vec![-2, 2]);
    let mut long_get_reader = BinaryReader::from_bytes(long_bytes, Endian::Little, true);
    assert_eq!(long_get_reader.get_vec_varint(0, 2).unwrap(), vec![-2, 2]);
...
fn read_enum_functions() {
    ...
    assert_get_enum!(vec![1], get_enum_u8, EnumU8);
