...
fn reserve_and_fill_bool() {
    ...
    writer.write_vec_u8(vec![0x00, 0x00, 0x00, 0x00]).unwrap();
...
fn write_varint_64bit() {
    ...
    big_vec_writer
        .write_vec_varint(vec![-2, 562949953421312])
        .unwrap();
    assert_eq!(
...
fn write_varint_i32() {
    ...
    big_vec_writer.write_vec_varint(vec![-2, 131072]).unwrap();
    assert_eq!(
        big_vec_writer.get_ref_bytes(),
        &[0xFF, 0xFF, 0xFF, 0xFE, 0x00, 0x02, 0x00, 0x00][..]
    );
