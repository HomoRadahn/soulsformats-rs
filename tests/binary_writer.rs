    ...
    ($name:ident, $type:ty, $write:ident, $write_vec:ident, $single:expr, $many:expr, $little_single:expr, $big_single:expr, $little_many:expr, $big_many:expr) => {
        ...
        fn $name() {
            for (endian, expected_single, expected_many) in [
                ...
                let mut writer = BinaryWriter::new(Cursor::new(Vec::new()), endian, true);
                writer.$write($single).unwrap();
                assert_eq!(writer.get_ref_bytes(), &expected_single[..]);

                let mut vec_writer = BinaryWriter::new(Cursor::new(Vec::new()), endian, true);
...
fn reserve_and_fill_bool() {
    let mut writer = BinaryWriter::from_bytes(Endian::Big, true);
    ...
    assert_eq!(writer.get_ref_bytes(), &vec![0x00, 0x00, 0x00, 0x00, 0x1, 0x00, 0x00, 0x00, 0x00]);
...
macro_rules! test_numeric_reservation {
    ($name:ident, $reserve:ident, $fill:ident, $single:expr, $little:expr, $big:expr, $expected_middle:expr) => {
        #[test]
        fn $name() {
            for (endian, expected_single) in [
                (Endian::Little, $little),
                (Endian::Big, $big),
            ] {
                let mut writer = BinaryWriter::from_bytes(endian, true);
                writer.write_u8_vec(&vec![0x00, 0x00, 0x00, 0x00]).unwrap();
...
test_numeric_reservation!(
    ...
    fill_u32,
    0x12345678,
    &vec![0x00, 0x00, 0x00, 0x00, 0x78, 0x56, 0x34, 0x12, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0x12, 0x34, 0x56, 0x78, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0xFE, 0xFE, 0xFE, 0xFE, 0x00, 0x00, 0x00, 0x00]
);
...
test_numeric_reservation!(
    ...
    fill_u64,
    0x12345678_12345678,
    &vec![0x00, 0x00, 0x00, 0x00, 0x78, 0x56, 0x34, 0x12, 0x78, 0x56, 0x34, 0x12, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0x12, 0x34, 0x56, 0x78, 0x12, 0x34, 0x56, 0x78, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0x00, 0x00, 0x00, 0x00]
);
...
test_numeric_reservation!(
    ...
    fill_i32,
    -3,
    &vec![0x00, 0x00, 0x00, 0x00, 0xFD, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFD, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0xFE, 0xFE, 0xFE, 0xFE, 0x00, 0x00, 0x00, 0x00]
);
...
test_numeric_reservation!(
    ...
    fill_i64,
    -3,
    &vec![0x00, 0x00, 0x00, 0x00, 0xFD, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFD, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0x00, 0x00, 0x00, 0x00]
);
...
test_numeric_reservation!(
    ...
    fill_f32,
    2.7,
    &vec![0x00, 0x00, 0x00, 0x00, 0xCD, 0xCC, 0x2C, 0x40, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0x40, 0x2C, 0xCC, 0xCD, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0xFE, 0xFE, 0xFE, 0xFE, 0x00, 0x00, 0x00, 0x00]
);
...
test_numeric_reservation!(
    ...
    fill_f64,
    2.7,
    &vec![0x00, 0x00, 0x00, 0x00, 0x9A, 0x99, 0x99, 0x99, 0x99, 0x99, 0x05, 0x40, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0x40, 0x05, 0x99, 0x99, 0x99, 0x99, 0x99, 0x9A, 0x00, 0x00, 0x00, 0x00],
    &vec![0x00, 0x00, 0x00, 0x00, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0xFE, 0x00, 0x00, 0x00, 0x00]
);
